  ...
  with tab4:
      ...
      st.caption(
          f'Looking for tickets with status = **"{st.session_state.status_filter}"**'
      )
      if st.button("🔍 Check Status Match"):
          if not st.session_state.api_key:
              ...
              if tickets:
                  st.success(
                      f'✅ Found **{len(tickets)}** ticket(s) with status "{st.session_state.status_filter}"'
                  )
                  rows = []
                  for t in tickets:
                      rows.append(
                  ...
                  st.dataframe(
                      pd.DataFrame(
                          rows,
                          columns=["#", "Subject", "Status", "Email", "Notified?"],
                      ),
                      use_container_width=True,
                      hide_index=True,
                  )
                  ...
                  st.error(
                      f'❌ No tickets found with status "{st.session_state.status_filter}"'
                  )
                  st.info(
                      "💡 Go to RepairShopr → Admin → Ticket Statuses and copy the exact name."
                  )
                  # Show all statuses found in recent tickets
      ...
      test_email = st.text_input(
          "Send test email to", placeholder="youremail@example.com"
      )
              ...
              with st.spinner(f"Sending test email to {test_email}..."):
                  try:
                      ...
                      with smtplib.SMTP("mail.illegear.com", 587) as server:
                          ...
                          server.sendmail(
                              "support@illegear.com", test_email, msg.as_string()
                          )
                      st.success(
                      ...
                      st.error(
                          "❌ Authentication failed — wrong email password. Check your credentials."
                      )
                      add_log("ERROR", "SMTP test failed: authentication error")
                  except smtplib.SMTPConnectError:
                      st.error(
                          "❌ Could not connect to mail.illegear.com:587 — server unreachable."
                      )
